...
class Mamba(nn.Module):
    ...
    def low_pass_filter_m_dim(self, input_tensor, cutoff_ratio=0.1):
        # input_tensor: (n, m, h) shape tensor
        # cutoff_ratio: percentage of frequencies to keep (low frequencies)

        # Perform Fourier transform along the m dimension (second dimension)
        fft_result = torch.fft.fft(input_tensor, dim=1)

        # Get the shape of the input tensor
        n, m, h = input_tensor.shape

        # Create a low-pass filter mask for the m dimension
        mask = torch.zeros_like(fft_result)

        # Define cutoff frequency for the m dimension
        m_cutoff = int(cutoff_ratio * m)

        # Apply the mask to retain low frequencies
        mask[:, :m_cutoff, :] = 1
        mask[:, -m_cutoff:, :] = 1
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            ...
            # # Example usage
            # input_tensor = torch.rand(128, 128, 64)  # Example input tensor with shape (n=128, m=128, h=64)
            dt = self.low_pass_filter_m_dim(dt, cutoff_ratio=0.1)

            # print(filtered_tensor.shape)  # Output shape should be the same as input
            # dt = self.fourier_proj_A(dt).squeeze(-1)
