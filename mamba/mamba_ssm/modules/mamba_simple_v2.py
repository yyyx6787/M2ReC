...
class Mamba(nn.Module):
    def __init__(
        ...
        self.conv = nn.Conv1d(128, 128, kernel_size =3, stride=1, padding=1)  #add pad
        
        self.conv1 = nn.Conv1d(128, 128, 5, stride=1, padding =(5 - 1) // 2)
        
        self.conv2 = nn.Conv1d(128, 128, 7, stride=1, padding =(7 - 1) // 2)
        self.trans_dim = nn.Linear(99,200)
        self.trans_dim1 = nn.Linear(294,200)
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            ...
            # print("check:", x.size(), dt.size(), A.size(), B.size())
            x = self.conv(x) + x
            # print("check dimention:", x_.size())
            # x_2 = self.conv1(x_1)
            # # print("check dimention:", x_.size())
            # x_3 = self.conv2(x_2)
            # x = x_1 + x_2 + x_3
            ...
            # x_1 = self.conv1(x)
            # x_2 = self.conv2(x)
            # x_cat = torch.cat((x_, x_1, x_2), dim=-1)
            # # x_pool = self.pool(x_cat)
            # x_1 = self.trans_dim(x_) 
            # print("check:", x_cat.size(),x_pool.size())
            # println()
            # x = self.alpha*x_1 + self.beta*x
            y = selective_scan_fn(
                x,
                dt,
                A,
            ...
            # y_ = self.conv(y)
            # y = self.trans_dim(y_) + y
            # y = self.trans_dim(y_) + x
            # x_ = self.conv(y)
            # x_1 = self.conv1(y)
            # x_2 = self.conv2(y)
            # x_cat = torch.cat((x_, x_1, x_2), dim=-1)
            # x_pool = self.pool(x_cat)
            # y = self.trans_dim1(x_cat)
